...
ARG USER_NAME=be-authentication
ARG USER_UID=1000
ARG USER_GID=${USER_UID}

RUN addgroup -g ${USER_GID} ${USER_NAME} \
    && adduser -h /opt/be-authentication -D -u ${USER_UID} -G ${USER_NAME} ${USER_NAME}

USER ${USER_NAME}
WORKDIR /opt/be-authentication
COPY --from=builder --chown=${USER_UID}:${USER_GID} /src/be-authentication/build/libs/*.jar app.jar
